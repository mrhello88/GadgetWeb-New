...
const ProductDetailPage = () => {
  ...
  useEffect(() => {
    // Set the first image as active when product data is loaded
    if (product && product.images && product.images.length > 0 && !activeImage) {
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 p-8">
            ...
            >
              {product.images && product.images.length > 0 ? (
                <div>
                  {/* Main Image Display */}
                  <div className="aspect-w-1 aspect-h-1 w-full overflow-hidden rounded-lg border-2 border-gray-100">
                    <img
                      src={failedImages[activeImage] ? '/placeholder.svg' : `http://localhost:5000/images/${activeImage}`}
                  ...
                  <div className="mt-4">
                    <div className="flex space-x-2 overflow-x-auto pb-2">
                      {product.images.map((img: string, index: number) => (
                        <div
                          key={index}
                          className={`w-20 h-20 flex-shrink-0 rounded-md overflow-hidden cursor-pointer border-2 transition-colors ${activeImage === img ? 'border-primary-500' : 'border-transparent hover:border-gray-300'}`}
                          onClick={() => setActiveImage(img)}
                        >
                          <img
                            src={failedImages[img] ? '/placeholder.svg' : `http://localhost:5000/images/${img}`}
            ...
            <div className="space-y-6">
              <div>
                ...
                {/* Rating - Show rating if available */}
                {(product.rating > 0 || product.reviewCount > 0) && (
                  ...
                  >
                    ...
                    <span className="text-sm text-gray-600">
                      {(product.rating || 0).toFixed(1)} ({product.reviewCount || 0} {(product.reviewCount || 0) === 1 ? 'review' : 'reviews'})
                    </span>
                  </motion.div>
                )}
                ...
                >
                  ...
                  <motion.button 
                    className="w-full px-6 py-3 bg-primary-500 text-white rounded-lg font-medium hover:bg-primary-600 transition-colors flex items-center justify-center group"
                    ...
                    onClick={() => {
                      try {
                        ...
                        window.location.href = `/compare/${product._id}`;
                      }
                    }}
                  >
                    <GitCompare className="h-5 w-5 mr-2 group-hover:rotate-12 transition-transform" />
                    Compare This Product
                  </motion.button>
                  
                  {/* Additional info text */}
                  <p className="text-xs text-gray-500 mt-2 text-center">
                    Compare this product with up to 2 other products
                  </p>
                </motion.div>
                        ...
                        >
                          <div className="flex items-start space-x-4">
                            <img
                              ...
                              src={failedImages[review.userAvatar] ? getProfileImageUrl(undefined) : getProfileImageUrl(review.userAvatar)}
                              alt={review.userName}
                              onError={() => handleImageError(review.userAvatar)}
                            />
              ...
              {relatedProducts.map((relatedProduct, index) => (
                ...
                >
                  <div className="h-48 overflow-hidden">
                    <img
                      src={
                        failedImages[relatedProduct._id]
                          ? '/placeholder.svg'
